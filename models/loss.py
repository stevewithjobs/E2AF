...
#     """
#     # correct = correct.permute(1, 0, 2)
#     out_planes = correct.size(2)
...
#     num_wrong = out_planes - correct.sum(dim=2, keepdim=True)
#     # Compute the label weight to give unselected experts.
#     unselected_weight = num_wrong / unselected
#     # Build the initial labels tensor.
#     # labels = unselected_weight.repeat(1, 1, num_experts, *([1]*(correct.ndim - 3)))
#     # labels = unselected_weight.repeat(1, 1, num_experts, *([1]*(correct.ndim - 2)))
#     labels = unselected_weight.repeat(1, 1, num_experts, 1, 1)
#     # Scatter the correct tensor into the labels tensor.
#     # This will set the entries that are correct / incorrect to be
#     # 0 / 1, respectively.
#     labels.scatter_(dim=2, index=routing_indices, src=correct)
#     return labels
...
def routing_classification_loss_by_error(net: Any,
    ...
    for module in net.modules():
        ...
        if hasattr(module, 'saved_error_signal'):
            ...
            saved_error_signal = module.saved_error_signal.norm(p=2, dim=-1)
            saved_error_signal = saved_error_signal.mean(dim=1).permute(0,2,1)
            routing_weights = module.routing_weights.mean(dim=1).permute(0,2,1)
            routing_indices = module.routing_indices.select(dim=1, index=0).permute(0,2,1)
            
            correct = get_correct_indices(
                saved_error_signal, quantile,
                unscale_quantity(saved_error_signal.numel() / 2, scaler)).to(
                    dtype=routing_weights.dtype)
            labels = construct_routing_labels(
                correct, routing_indices,
                module.smoe_config.num_experts)
            loss = F.binary_cross_entropy_with_logits(
