...
#     tensor = tensor.sum(-1) # 将特征求和
#     q = tensor.reshape(a, b, -1).to(dtype=torch.float32).quantile(quantile, dim=2)
#     # Reshape so broadcasting works correctly.
#     # q = q.reshape(tensor.size(0),tensor.size(1), *([1]*(tensor.ndim - 2)))
#     q = q.reshape(a, b, 1, 1)
...
#     result = result.unsqueeze(-1).repeat(1, 1, 1, 1, f)
#     return result
